    ...
    def collision_check(self, paths, obstacles):
        ...
        for i in range(len(paths)):
            ...
            for j in range(len(path[0])):
                ...
                # circle_locations[:, 1] = ...
                # pdb.set_trace()
                for dummy in range(len(self._circle_offsets)):
                    circle_locations[dummy, 0] = path[0][j] + self._circle_offsets[dummy] * cos(path[2][j])  
                    circle_locations[dummy, 1] = path[1][j] + self._circle_offsets[dummy] * sin(path[2][j])                                      
                ...
                for k in range(len(obstacles)):
                    ...
                    # pdb.set_trace()
                    # collision_dists = \
                    #     scipy.spatial.distance.cdist(obstacles[0][k], 
                    #                                  circle_locations)                        
                ...
                for j in range(len(paths)):
                    if j == i:
                        ...
                        if not collision_check_array[j]:
                            ...
                            # --------------------------------------------------
                            # score += self._weight * ...
                            score += self._weight * (-sqrt((paths[i][0][-1]-paths[j][0][-1])**2+ (paths[i][1][-1]-paths[j][1][-1])**2))
                            # --------------------------------------------------
