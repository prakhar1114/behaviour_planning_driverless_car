...
class PathOptimizer:
    ...
    def optimize_spiral(self, xf, yf, tf):
        ...
        # pdb.set_trace()
        res = scipy.optimize.minimize(self.objective, p0, method='L-BFGS-B', jac= self.objective_grad, constraints=bounds,options={'disp':False})
        # ------------------------------------------------------------------
        # pdb.set_trace()
    ...
    def thetaf(self, a, b, c, d, s):
        ...
        s3=np.multiply(s,s2)
        s4=np.multiply(s,s3)        
        thetas = np.multiply(a,s) + 0.5*np.multiply(b,s2) + np.multiply(c,s3)/3 + np.multiply(d,s4)/4
        return thetas
        # ------------------------------------------------------------------
