...
class BehaviouralPlanner:
    ...
    def transition_state(self, waypoints, ego_state, closed_loop_speed):
        ...
        if self._state == FOLLOW_LANE:
            ...
            # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
            # ------------------------------------------------------------------
            # goal_index = get_goal_index(self, waypoints, ego_state, closest_len, closest_index)
            ...
            # ------------------------------------------------------------------
            if stop_sign_found:
              self._goal_state = [waypoints[self._goal_index][0],waypoints[self._goal_index][1],0]
        ...
        # the stop sign and transition to the next state.
            ...
            if self._stop_count == STOP_COUNTS:
                ...
                goal_index_stop, stop_sign_found = self.check_for_stop_signs(waypoints, closest_index, goal_index)                
                self._goal_index = goal_index 
                self._goal_state = waypoints[self._goal_index] 
                # self._stop_count=0
                # self._state = FOLLOW_LANE
    ...
    def get_goal_index(self, waypoints, ego_state, closest_len, closest_index):
        ...
        while dummy < len(waypoints) - 1:
          dummy=dummy+1
          arc_length =arc_length + math.sqrt((waypoints[dummy][1]-waypoints[dummy-1][1])**2+(waypoints[dummy][0]-waypoints[dummy-1][0])**2)
          if arc_length> self._lookahead:
            wp_index=dummy
            break
        # ------------------------------------------------------------------
...
def get_closest_index(waypoints, ego_state):
    ...
    x=ego_state[0]
    y=ego_state[1]
    closest_len=1000000 #some large number (will be updated)
    cur_dist=900000 #some random value
    for i in range(len(waypoints)):
        prev_cur_dist=cur_dist
        cur_dist= math.sqrt((x-waypoints[i][0])**2+(y-waypoints[i][1])**2)
        
        if closest_len< cur_dist and closest_len<=prev_cur_dist:
            break    

        if cur_dist<=closest_len:
            closest_len=cur_dist
            closest_index=i
